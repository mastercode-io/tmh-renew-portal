...
const CONFIRMATION_URL = '/confirmation.html';
const OFFER_FALLBACK_URL = '/';
...
function setPayNowButtonMode(mode = 'default', labelText = '') {
  ...
  if (mode === 'loading' || mode === 'waiting') {
    const text =
      labelText || (mode === 'loading' ? 'Processing your order...' : 'Waiting for payment...');
    btn.disabled = true;
...
function rememberOfferUrl() {
  try {
    ...
    if (referrer && referrer !== window.location.href) {
      sessionStorage.setItem(OFFER_URL_STORAGE_KEY, referrer);
    } else if (!sessionStorage.getItem(OFFER_URL_STORAGE_KEY)) {
      sessionStorage.setItem(OFFER_URL_STORAGE_KEY, OFFER_FALLBACK_URL);
    }
  } catch (error) {
...
function getOfferUrl() {
  try {
    return sessionStorage.getItem(OFFER_URL_STORAGE_KEY) || OFFER_FALLBACK_URL;
  } catch (error) {
...
function clearPaymentSessionData() {
  try {
    sessionStorage.removeItem(ORDER_DATA_STORAGE_KEY);
...
function buildStatusAction(action) {
  switch (action) {
    ...
    case 'contact': {
      ...
      link.href = `mailto:${CONTACT_SUPPORT_EMAIL}?subject=Order%20payment%20support`;
...
async function performStatusCheck(options = {}) {
  ...
  try {
    const response = await fetch(`${PAYMENT_STATUS_ENDPOINT}?token=${encodeURIComponent(paymentState.token)}`, {
      method: 'GET',
      credentials: 'include',
      cache: 'no-store'
    });
  ...
  if (payNowBtn && termsCheckbox) {
    ...
    payNowBtn.addEventListener('click', async function(e) {
      ...
      try {
        ...
        const response = await fetch(`${PAYMENT_LINK_ENDPOINT}?token=${encodeURIComponent(dealToken)}`, {
          method: 'GET',
          credentials: 'include'
        });
