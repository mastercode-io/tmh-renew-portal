...
| `POST` | `/api/renewal/order` | Converts the renewal submission into CRM entities and responds with the order summary. | `CRM_REQUIRED_APIS.RenewalOrder` |
...
| `GET` | `/api/renewal/:dealId/payment-link` | Requests a hosted payment URL (CRM custom API triggers Xero actions). | `CRM_REQUIRED_APIS.XeroPaymentLink` |
